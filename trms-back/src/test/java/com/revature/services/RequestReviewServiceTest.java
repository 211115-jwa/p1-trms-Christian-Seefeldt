...
public class RequestReviewServiceTest {
	...
	private static Set<Reimbursement> req;
	private static Set<Employee> emps;
	private static Set<Comment> com;
	...
	public void getPendingReimbursements() {
		List<Employee> emp = new ArrayList<>(emps);
		
		when(remDAO.getByRequestor(emp.get(0))).thenReturn(req);
		Set<Reimbursement> rem = rrServ.getPendingReimbursements(emp.get(1));
